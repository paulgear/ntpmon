...
def format_tags(metrics: dict, additional_tags: dict) -> str:
    return ','.join([
        f'{transform_identifier(tag)}={metrics[tag]}' for tag in sorted(metrics.keys()) if tag not in exclude_tags and type(metrics[tag]) == str
    ] + [
        f'{transform_identifier(tag)}={additional_tags[tag]}' for tag in sorted(additional_tags.keys()) if tag not in exclude_tags
    ])
